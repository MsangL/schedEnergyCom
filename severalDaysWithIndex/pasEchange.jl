 ...
 function pasEchange(TliP::Number)
      mod=Model(CPLEX.Optimizer)
      set_optimizer(mod, optimizer_with_attributes( CPLEX.Optimizer,"CPX_PARAM_TILIM" =>TliP))
      ...
      @constraint(mod,[ j in 1:JA, i in 1:n , t in 1:T,k in 1:Nbp,l in 1:u], sum(xa[j,i,k,t,s,l] for s in 1:pow)<=1) #
      @constraint(mod, [ t in 1:T, i in 1:n,k in 1:Nbp,l in 1:u], y[i,k,t,1,l]==y[i,k,t-1,1,l]+(delta/Cm[k,i])*(1000*sum(Pa[i,1,k,s]*xa[1,i,k,t,s,l] for s in 1:pow)-U[k,i]*(y[i,k,t-1,1,l]-Yout[l,t]))) # contrainte température maison
      ...
      @constraint(mod, [ i in 1:n, k in 1:Nbp,l in 1:u,  t in 1:T;t!=tt], y[i,k,t,2,l]==(y[i,k,t-1,2,l]*r*cpo*Masse[k,i]+S[k,i]*K*dist*r*Ta+ sum(Pa[i,2,k,s]*xa[2,i,k,t,s,l] for s in 1:pow)*dist*860)/(Masse[k,i]*cpo*r+S[k,i]*dist*K*r)) # température eau 
      ...
      @constraint(mod,[i in 1:n,j in 1:JA,k in 1:Nbp,l in 1:u],sum(xa[j,i,k,t,p,l] for t in 1:T,p in 1:pow)<=T*JobA[i,j,k,l])           # en parler avc Michael
      ...
      @constraint(mod,[j in 1:n,i in 1:n,t in 1:T,l in 1:u],f[j,i,t,l]==0)                                                    # f_ij=0
      @constraint(mod,[i in 1:n,l in 1:u],Gain[i,l]==(Pv*V[i,l]-Pedf*C[i,l]))                                           # gain avant
